...
def test_uv_pip_install_simple_package(temp_project):
    ...
    result = subprocess.run(
        ["uv", "pip", "install", "--python", str(venv_path / "bin" / "python"), "six"],
        capture_output=True,
        text=True,
    )
...
def test_uv_is_faster_than_pip(temp_project):
    ...
    result = subprocess.run(
        ["uv", "pip", "install", "--python", str(venv_path / "bin" / "python"), "requests"],
        capture_output=True,
        text=True,
    )
...
def test_uv_cache_reuse(temp_project):
    ...
    result1 = subprocess.run(
        ["uv", "pip", "install", "--python", str(venv1 / "bin" / "python"), "click==8.1.7"],
        capture_output=True,
        text=True,
        check=True,
    )
    ...
    result2 = subprocess.run(
        ["uv", "pip", "install", "--python", str(venv2 / "bin" / "python"), "click==8.1.7"],
        capture_output=True,
        text=True,
        check=True,
    )
    ...
    for venv in [venv1, venv2]:
        check = subprocess.run(
            [str(venv / "bin" / "python"), "-c", "import click"], capture_output=True
        )
...
def test_uv_handles_dependencies():
    ...
    try:
        ...
        result = subprocess.run(
            ["uv", "pip", "install", "--python", str(venv_path / "bin" / "python"), "flask==3.0.0"],
            capture_output=True,
            text=True,
            check=True,
        )
    ...
    def test_student_understands_pubgrub(self):
        ...
        if solution_dir.exists():
            ...
            if docs:
                ...
                assert "pubgrub" in content, "Documentation should mention PubGrub"
    ...
    def test_student_understands_rust_benefits(self):
        ...
        if solution_dir.exists():
            ...
            if docs:
                ...
                assert mentions >= 2, "Documentation should explain Rust advantages"
