...
def compare_installation():
    ...
    pip_time += measure_command(
        ["python", "-m", "venv", str(pip_venv)], "pip: create virtual environment"
    )
    pip_time += measure_command(
        [str(pip_venv / "bin" / "pip"), "install", "requests"], "pip: install requests"
    )

    # Test with uv
    uv_time = 0
    uv_time += measure_command(["uv", "venv", str(uv_venv)], "uv: create virtual environment")
...
def show_architecture():
    ...
    result = subprocess.run(
        ["uv", "cache", "prune", "--dry-run"], capture_output=True, text=True, check=False
    )
    print("\nCache information:")
